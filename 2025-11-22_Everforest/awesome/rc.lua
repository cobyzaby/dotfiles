...
screen.connect_signal("property::geometry", function(s)
	...
	if beautiful.wallpaper then
		...
		elseif wallpaper == "random" then
			local x = math.random(#beautiful.wallpaper_presets)
			wallpaper = beautiful.wallpaper_presets[x]
			if type(wallpaper) == "function" then
				wallpaper = wallpaper(s)
			else
...
globalkeys = mytable.join(
	...
	awful.key({ altkey, "Control" }, "l", function()
		awful.spawn("/home/toads/.config/scripts/lock.sh")
	...
	awful.key({ altkey }, "m", function()
		...
		if beautiful.a_status == "on" then
			beautiful.a_status = "off"
		else
			beautiful.a_status = "on"
		end
		beautiful.volume.update()
...
awful.rules.rules = {
	...
	-- clients we don't want having titlebars or borders
	{
		rule_any = {
			class = {
				"news-flash",
	...
	-- Set Discord to always map on the tag named "5"
	{ rule = { class = "discord" }, properties = { screen = s, tag = "5" } },
}
